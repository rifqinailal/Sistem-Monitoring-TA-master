async function initGraduatedGraph() {
    const res = await fetch(`${BASE_URL}/apps/dashboard/get-graduated-data`);
    ...
    if (res.status == 200) {
        ...
        var chart = new ApexCharts(
            document.querySelector("#graduatedGraph"),
            options
        );
        chart.render();
        return;
    ...
    swal.fire({
        title: "Oops!",
        text: "Gagal memuat data kelulusan",
        type: "warning",
    });
}

async function initStudentGraph() {
    const res = await fetch(`${BASE_URL}/apps/dashboard/get-student-data`);
    ...
    if (res.status == 200) {
        ...
        var chart = new ApexCharts(
            document.querySelector("#studentGraph"),
            options
        );
        chart.render();
        return;
