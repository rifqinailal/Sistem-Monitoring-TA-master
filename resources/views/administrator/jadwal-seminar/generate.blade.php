...
@section('content')
    <div class="card">
        <div class="card-body">
            ...
            <form action="{{ route('apps.jadwal-seminar.generate-auto') }}" method="POST" id="formGenerate">
                @csrf
                ...
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <h5 class="card-title m-0">Daftar Mahasiswa Menunggu Penjadwalan</h5>
                    <div>
                        <a href="{{ route('apps.jadwal-seminar') }}" class="btn btn-secondary"><i class="bx bx-x"></i> Batal</a>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <ul class="ps-3 mb-0 small">
                                            @foreach ($item->tugas_akhir->bimbing_uji->where('jenis', 'penguji')->sortBy('urut') as $dosen)
                                                <li>{{ $dosen->dosen->name ?? '-' }} (PU {{ $dosen->urut }})</li>
                                            @endforeach
                                        </ul>
                                    </td>
                                </tr>
...
<script>
    $(document).ready(function() {
        ...
        // 2. Logika Select All menembus semua halaman (page)
        $('#selectAll').on('click', function() {
            var rows = table.rows({ 'search': 'applied' }).nodes();
            $('input[type="checkbox"].item-checkbox', rows).prop('checked', this.checked);
        });

        // 3. Trigger saat tombol submit ditekan
        $('#formGenerate').on('submit', function(e) {
            ...
            // AKALAN PENTING: Sisipkan input hidden untuk checkbox di halaman yang tersembunyi (page 2, dst)
