                    ...
                    <form action="{{route('apps.jadwal-sidang.update', ['jadwalSidang' => $jadwalSidang->id])}}" method="post">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="pengganti2" class="form-control select2 dosen-select" onchange="updateOptions()">
                                <option value="">Pilih Dosen Penguji Pengganti</option>
                                @foreach ($pengujiPengganti as $item)
                                    <option value="{{ $item->id }}" {{isset($editedData) && !is_null($editedData->tugas_akhir->bimbing_uji()->where('jenis',  'pengganti')->where('urut', 2)->first()) && $editedData->tugas_akhir->bimbing_uji()->where('jenis',  'pengganti')->where('urut', 2)->first()->dosen_id == $item->id || old('pengganti2') == $item->id ? "selected" : ''}} {{isset($editedData) && !is_null($editedData->tugas_akhir->bimbing_uji()->where('jenis',  'pengganti')->where('urut', 1)->first()) && $editedData->tugas_akhir->bimbing_uji()->where('jenis',  'pengganti')->where('urut', 1)->first()->dosen_id == $item->id || old('pengganti2') == $item->id ? "disabled" : ''}}>
                                        {{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.slot-rekomendasi', function(e) {
                ...
                // Isi input form langsung menggunakan selector name atribut
                ...
                // Efek scroll layar kembali ke form atas
                $('html, body').animate({
